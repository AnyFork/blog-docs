<template>
  <div class="icon-link inline-flex items-center" v-if="!type">
    <a :href="link" :target="target" v-if="link || text" class="inline-flex items-center" style="color: inherit">
      <client-only>
        <component :is="componentId" :key="icon" :style="iconStyle"></component>
      </client-only>
...
const componentId = defineAsyncComponent(() => import(/* @vite-ignore */ `../../../../../node_modules/@vicons/antd/es/${props.icon}.js`))
</script>
