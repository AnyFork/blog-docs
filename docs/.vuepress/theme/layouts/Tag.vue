    ...
    <template #page>
      <main class="page">
        <div class="pt-[20px] w-[var(--content-width)] my-0 mx-auto">
          <Icon icon="AppstoreTwotone" :iconSize="25" text="标签列表" :textSize="20" class="dark:hover:text-[#fff]" />
          <div class="w-full flex my-[10px]">
            <RouterLink v-for="({ items, path }, name) in tagMap.map" :key="name" :to="path" class="shadow-item flex items-center text-[#666] px-[14px] font-normal h-[50px] bg-[#fff] hover:bg-[#3eaf7c] hover:text-[#fff] dark:bg-[#181818] dark:hover:bg-[#3eaf7c] dark:text-[#fff] rounded mx-[5px]">
              <span>{{ name }}</span>
